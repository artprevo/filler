...
/*   Updated: 2019/10/23 14:04:00 by artprevo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "filler.h"

static	void		finishdom(t_dominance *dom)
{
	if (dom->z2 < dom->z3)
	{
		if (dom->z2 < dom->z4)
...
static	int			zone(t_env *env, t_dominance *dom, int xdest, int ydest)
{
	t_linetab	*tab;
	int			i;
	int			sum;
...
{
	dom->z1 = zone(env, dom, env->xtab / 2, (env->ytab / 2) + 4);
	dom->ystart = (env->ytab / 2) + 4;
	dom->z2 = zone(env, dom, env->xtab / 2, env->ytab + 4);
	dom->ystart = 4;
	dom->xstart = env->xtab / 2;
	dom->z3 = zone(env, dom, env->xtab, (env->ytab / 2) + 4);
	dom->ystart = (env->ytab / 2) + 4;
	dom->z4 = zone(env, dom, env->xtab, env->ytab + 4);
}
